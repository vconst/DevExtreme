...
QUnit.test("Correct start scroll position when RTL", function(assert) {
    ...
    var ie8Correction = browser.msie && parseInt(browser.version) === 8 ? 1 : 0,
        scrollLeft;
    ...
    assert.equal(scrollLeft, 100 + ie8Correction);
...
QUnit.test("Correct start scroll position when RTL and detached container of the datagrid", function(assert) {
    ...
    var clock = sinon.useFakeTimers(),
        ...
        ie8Correction = browser.msie && parseInt(browser.version) === 8 ? 1 : 0,
        scrollLeft;
...
QUnit.test("Scroll position after grouping when RTL", function(assert) {
    ...
    //assert
    assert.equal(scrollable.scrollLeft(), 300 + ie8Correction, "scroll position");
    clock.restore();
...
QUnit.test("Hide group panel and search panel when calculateDisplayValue is defined", function(assert) {
    ...
    var visibleColumns,
        dataGrid = createDataGrid({
            ...
            columns: [{ dataField: "field1", calculateDisplayValue: $.noop, groupIndex: 0 }, { dataField: "field2", groupIndex: 1 }],
            groupPanel: { visible: true }
        });
